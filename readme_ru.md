...
На первый раз нам все-же придется скачать эту библиотеку через [pip](https://pip.pypa.io/en/stable/installation/). Откроем папку нашего проекта в терминале, и введем в терминал эту команду:

```sh
pip install requests -t project_libs_dir
```

Можете запустить эту команду еще раз, чтобы установить другие нужные библиотеки. `requests` - название библиотеки, которую мы устанавливаем, а `project_libs_dir` - название папки, в которую устанавливаем библиотеку. После запуска этой команды создастся папка `project_libs_dir`, а в нее установится библиотека `requests` и все ее зависимости нужных версий:
...
Теперь давайте добавим все содержимое папки `project_libs_dir` в архив, который назовем `project_libs.zip` и положим в папку вашего проекта. Очень важно не перепутать и добавить именно содержимое папки `project_libs_dir`, а не саму папку. Когда вы откроете архив, он должен выглядеть вот так:
...
Теперь попробуем запустить этот код:
...
Если запуск на дал никакого вывода и никаких ошибок, то все в порядке. Теперь зарегистрируйтесь на [github](https://github.com) и создайте там новый репозиторий, пусть имя вашего аккаунта будет `username`, а название репозитория `project` - такое же, как название папки вашего проекта.
